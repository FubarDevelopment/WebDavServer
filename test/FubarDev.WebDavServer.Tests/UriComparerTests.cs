...
namespace FubarDev.WebDavServer.Tests
{
    public class UriComparerTests
    {
        ...
        public void CompareSameHosts(string otherUrl)
        {
            ...
            Assert.True(comparer.IsSameHost(new Uri("http://localhost/"), new Uri(otherUrl, UriKind.RelativeOrAbsolute)));
        }
        ...
        {
            var comparer = new DefaultUriComparer();
            Assert.False(comparer.IsSameHost(new Uri("http://localhost/"), new Uri(otherUrl, UriKind.RelativeOrAbsolute)));
        }
